...
public class MiniGame1 implements Screen
{
    ...
    private mx.izo.proyecto.Personaje H;
    ...
    ArrayList<mx.izo.proyecto.BalaV> balasL = new ArrayList<mx.izo.proyecto.BalaV>();
    ...
    private void crearObjetos() {
        ...
        // Crear el personaje
        H = new mx.izo.proyecto.Personaje(texturaPersonaje,texturaSalto,texturaPersonaje2);
            ...
            for (mx.izo.proyecto.EnemigoV enemigoV : enemigosV) {
                if (enemigoV.getVidas() > 0) {
                    ...
                    if ((int) tiempoJuego == 1 && banderaDisparo) {
                        mx.izo.proyecto.BalaV balaEnJuego = new mx.izo.proyecto.BalaV(texturaManzanas);
                    ...
                    for (mx.izo.proyecto.BalaV bala : balasL) {
                        bala.render(batch);
                        banderaDisparo = true;
    ...
    private void moverPersonaje() {
        ...
        switch (H.getEstadoMovimiento()) {
            case INICIANDO:     // Mueve el personaje en Y hasta que se encuentre sobre un bloque
                ...
                if (celda==null) {
                    ...
                    H.setEstadoMovimiento(mx.izo.proyecto.Personaje.EstadoMovimiento.QUIETO);
                }
        ...
        if (H.getEstadoMovimiento()!= mx.izo.proyecto.Personaje.EstadoMovimiento.INICIANDO
                && (H.getEstadoSalto() != mx.izo.proyecto.Personaje.EstadoSalto.SUBIENDO) ) {
            // Calcula la celda donde estaría después de moverlo
            int celdaX = (int) (H.getX() / TAM_CELDA);
            int celdaY = (int) ((H.getY() + H.VELOCIDAD_Y) / TAM_CELDA);
            // Recuperamos la celda en esta posición
            ...
            if ( (celdaAbajo==null && celdaDerecha==null) || esCoin(celdaAbajo) || esCoin(celdaDerecha) ) {
                // Celda vacía, entonces el personaje puede avanzar
                H.caer();
                H.setEstadoSalto(mx.izo.proyecto.Personaje.EstadoSalto.CAIDA_LIBRE);
            }
                ...
                // Dejarlo sobre la celda que lo detiene
                H.setPosicion(H.getX(), (celdaY + 1) * TAM_CELDA);
                H.setEstadoSalto(mx.izo.proyecto.Personaje.EstadoSalto.EN_PISO);
    ...
    private void probarChoqueParedes() {
        mx.izo.proyecto.Personaje.EstadoMovimiento estado = H.getEstadoMovimiento();
        ...
        px = H.getEstadoMovimiento()== mx.izo.proyecto.Personaje.EstadoMovimiento.MOV_DERECHA? px+H.getVelocidadPersonaje():
                px-H.getVelocidadPersonaje();
        int celdaX = (int)(px/TAM_CELDA);   // Casilla del personaje en X
        if (H.getEstadoMovimiento()== mx.izo.proyecto.Personaje.EstadoMovimiento.MOV_DERECHA) {
            ...
            if ( esCoin(capaPlataforma.getCell(celdaX,celdaY)) ) {
                ...
                Timer.schedule(new Timer.Task() {
                    ...
                    public void run() {
                        ...
                        plataforma.setScreen(new mx.izo.proyecto.PantallaCargando2_A(plataforma));
                    }
                }, 3);  // 3 segundos
        ...
        if ( capaPlataforma1.getCell(celdaX,celdaY) != null || capaPlataforma1.getCell(celdaX,celdaY+1) != null ) {
            if ( esPuertaA( capaPlataforma1.getCell(celdaX,celdaY) ) ) {
                ...
                Timer.schedule(new Timer.Task() {
                    @Override
                    public void run() {
                        plataforma.setScreen(new mx.izo.proyecto.PantallaGanaste(plataforma));
                    }
                }, 3);  // 3 segundos
            }
        }
    ...
    public class ProcesadorEntrada extends InputAdapter
    {
        ...
        public boolean touchDown(int screenX, int screenY, int pointer, int button) {
            ...
            if (estadoJuego==EstadosJuego.JUGANDO) {
                ...
                if (btnDerecha.contiene(x, y) && H.getEstadoMovimiento() != mx.izo.proyecto.Personaje.EstadoMovimiento.INICIANDO) {
                    ...
                    H.setEstadoMovimiento(mx.izo.proyecto.Personaje.EstadoMovimiento.MOV_DERECHA);

                } else if (btnIzquierda.contiene(x, y) && H.getEstadoMovimiento() != mx.izo.proyecto.Personaje.EstadoMovimiento.INICIANDO) {
                    // Tocó el botón izquierda, hacer que el personaje se mueva a la izquierda
                    banderaDireccion = true;
                    H.setBanderaPosicion(banderaDireccion);
                    H.setEstadoMovimiento(mx.izo.proyecto.Personaje.EstadoMovimiento.MOV_IZQUIERDA);
        ...
        public boolean touchUp(int screenX, int screenY, int pointer, int button) {
            ...
            if (H.getEstadoMovimiento()!= mx.izo.proyecto.Personaje.EstadoMovimiento.INICIANDO && (btnDerecha.contiene(x, y) || btnIzquierda.contiene(x,y)) ) {
                // Tocó el botón derecha, hacer que el personaje se mueva a la derecha
                H.setEstadoMovimiento(mx.izo.proyecto.Personaje.EstadoMovimiento.QUIETO);
            }
            return true;    // Indica que ya procesó el evento
        ...
        public boolean touchDragged(int screenX, int screenY, int pointer) {
            ...
            if (x<Plataforma.ANCHO_CAMARA/2 && H.getEstadoMovimiento()!= mx.izo.proyecto.Personaje.EstadoMovimiento.QUIETO) {
